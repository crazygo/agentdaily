    ...
    for date in dates:
        ...
        try:
            with open(f"{base_path}/whitelist-updates.json", 'r') as f:
                ...
                for item in data:
                    item['_source_date'] = date
                    item['_source_file'] = 'whitelist-updates'
                    all_items.append(item)
        except:
            pass
...
def categorize_item(item):
    ...
    # 1. New Technologies: Papers, research, fundamental models/frameworks
    tech_indicators = [
        'arxiv', 'paper', 'research', 'benchmark', 'evaluation',
        'llm', 'tokenizer', 'architecture', 'framework', 'algorithm',
        'neural network', 'modeling', 'theorem', 'latent'
    ]

    if any(ind in combined for ind in tech_indicators):
        # But check if it's actually a product update
        if 'release' not in combined and 'v2.' not in combined and 'v3.' not in combined and 'v4.' not in combined:
            return 'New Technologies'

    # 2. New Products: Complete new tools/apps/agents
    # Check if from new-product.json
    if source_file == 'new-product':
        return 'New Products'

    # Check for product launch indicators
    product_indicators = [
        'launch', 'introducing', 'announces', 'released', 'hits',
        'reached', 'valuation', 'startup', 'founded'
    ]
    if any(ind in combined for ind in product_indicators):
        return 'New Products'

    # 3. New Features: Updates to existing tools
    feature_indicators = [
        'update', 'version ', ' v', 'integration', 'feature',
        'changelog', 'improvement', 'fix', 'enhancement',
        'now available', 'rolling out', 'adds'
    ]
    if any(ind in combined for ind in feature_indicators):
        return 'New Features'

    # Check for product names + version/update
    product_names = ['github', 'copilot', 'cursor', 'windsurf', 'claude', 'aider', 'continue']
    if any(prod in combined for prod in product_names):
        if any(ind in combined for ind in ['v2', 'v3', 'v4', 'v5', '0.', '1.', '2.', '3.', '4.', '5.']):
            return 'New Features'

    # 4. Others: News, commentary, general discussions
    other_indicators = [
        'discussion', 'community', 'says', 'warns', 'opinion',
        'analysis', 'thoughts', 'debate', 'controversy'
    ]
    if any(ind in combined for ind in other_indicators):
        return 'Others'

    # Default based on source
    if source_file == 'whitelist-updates':
        return 'New Features'

    if source_file == 'insights':
        # Check if it's about industry news vs technical content
        if 'techcrunch' in description or 'the verge' in description or 'bloomberg' in description:
            if 'investment' in combined or 'funding' in combined or 'acquisition' in combined:
                return 'Others'
        return 'New Technologies'
